...
app.get('/api/agent/:agentId/missions', requireRole('MEMBER', 'AGENT', 'ADMIN'), (req, res, next) => {
  try {
    res.json({ missions: guildServer.missionEngine.getMissionsByAgent(req.params.agentId) });
  } catch (error) {
    next(error);
  }
});
...
app.get('/admin-api/missions', requireAdmin, (_req, res, next) => {
  try {
    const missions = guildServer.missionEngine.getAllActiveMissions();
...
app.post('/api/quests/agent-publish', requireRole('AGENT', 'ADMIN'), agentActionRateLimit(10), (req, res, next) => {
  try {
    ...
    const agent = guildServer.getGuildSnapshot().agents.find((candidate) => candidate.did === actorDid);
    if (!agent) {
      throw new HttpError(403, 'AGENT_DID_REQUIRED', 'Authenticated DID must belong to a guild agent');
    }
    if (!guildServer.checkAgentActionRateLimit(agent.id)) {
      throw new HttpError(429, 'AGENT_RATE_LIMIT', 'Agent action rate limit exceeded: 10 per hour');
    }

    const quest = guildServer.publishAgentInitiatedQuest({
      title,
      description,
      ...
      triggeredBy,
      sourceMissionId: typeof req.body?.sourceMissionId === 'string' ? req.body.sourceMissionId : undefined,
    });
...
app.post('/admin-api/revoke-agent-action/:questId', requireAdmin, (req, res, next) => {
  try {
    const quest = guildServer.revokeAgentInitiatedQuest(req.params.questId, req.principal?.did, req.principal?.role);
    if (!quest) {
      res.status(404).json({ error: 'QUEST_NOT_FOUND', message: 'Quest not found' });
      return;
    }
