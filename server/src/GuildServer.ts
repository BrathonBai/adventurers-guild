...
export class GuildServer {
  ...
  constructor(port: number = 3000, bindHost: string = '0.0.0.0', publicHost: string = 'localhost') {
    ...
    this.missionEngine.setSnapshotProvider(() => this.state.createSnapshot());
  ...
  private handleRegisterMissions(ws: WebSocket, message: IncomingMessage): void {
    ...
    const created = missions
      .map((mission) => this.readMissionPayload(mission))
      .filter((mission): mission is AgentMissionPayload => Boolean(mission))
      .map((mission) => this.missionEngine.registerMission(agent.id, mission));
